...
# Function to plot for a species
def plot_species_histogram(species):
    ...
    iqr_val = np.percentile(species_data['flipper_length_mm'], 75) - np.percentile(species_data['flipper_length_mm'], 25)
    lower_quartile = np.percentile(species_data['flipper_length_mm'], 25)
    upper_quartile = np.percentile(species_data['flipper_length_mm'], 75)

    # Create a histogram
    fig = px.histogram(species_data, x='flipper_length_mm', nbins=30, title=f"Flipper Length of {species}")
    
    # Add lines for mean and median
    fig.add_vline(x=mean_val, line=dict(color='blue', dash='dash'), annotation_text="Mean", annotation_position="top left")
    fig.add_vline(x=median_val, line=dict(color='green', dash='dot'), annotation_text="Median", annotation_position="top left")
    
    # Add rectangles for range, IQR, and ±2 standard deviations
    fig.add_vrect(x0=min_val, x1=max_val, fillcolor="lightgray", opacity=0.3, line_width=0, annotation_text="Range")
    fig.add_vrect(x0=lower_quartile, x1=upper_quartile, fillcolor="lightblue", opacity=0.3, line_width=0, annotation_text="IQR")
...
for species in species_list:
    fig = plot_species_histogram(species)
    fig.show()


# Chat:https://chatgpt.com/share/66f19181-89d4-800e-850c-f74bc9dc2521
...
# Function to plot KDE with annotations for a species
def plot_species_kde(species, ax):
    # Filter data by species
    ...
    lower_quartile = np.percentile(species_data, 25)
    upper_quartile = np.percentile(species_data, 75)
    ...
    ax.axvspan(lower_quartile, upper_quartile, color='lightblue', alpha=0.3, label='IQR')
...
# Iterate over species and create a KDE plot for each in a different subplot
for i, species in enumerate(species_list):
    plot_species_kde(species, axes[i])

# Add common labels and legend
axes[1].legend(loc='upper left')  # Legend on the middle plot
fig.suptitle('Flipper Length KDE Plots with Marked Statistics', fontsize=16)
plt.tight_layout()
...
# # Summary
# 1. **Initial Task with Plotly Histograms**:
#    - We discussed how to visualize the distribution of `flipper_length_mm` for each penguin species (`Adelie`, `Chinstrap`, `Gentoo`) using Plotly histograms. You wanted to mark specific statistical locations and scales (mean, median, range, IQR, and ±2 standard deviations) on the histograms.
#    - We used `fig.add_vline()` for marking the mean and median as vertical lines and `fig.add_vrect()` to shade the areas for range, IQR, and ±2 standard deviations.
# 
# 2. **Seaborn KDE Plot Version**:
#    - We repeated the same task, but using Seaborn’s Kernel Density Estimation (KDE) plots instead of histograms, and organized the output in a row of three plots, one for each species.
#    - We used `ax.axvline()` to mark mean and median as vertical lines, and `ax.axvspan()` to shade the regions for range, IQR, and ±2 standard deviations.
# 
# Both approaches allowed us to visualize key descriptive statistics over the distribution of `flipper_length_mm` for each penguin species, highlighting the location and scale of the data. The result was visual plots that clearly showed both the shape of the data distribution and important summary statistics for each species.
...
# My preference is kernel density estimators. All of them have advantages and disadvantages. For box plots, it is best for understanding the data's spread and spotting outliers easily without focusing too much on the shape of the distribution. But box plot only summarize key statistics so it always lose many details.Box plot have less sence distribution shap. For Histogram, it is best for visualizing the spread and distribution of numerical data, including skewness and modalit. Histogram is hard to compare data especislly data overlaps. Although KDEs are lack in the exact numbers, it can compare data easily and offer a smooth view to show the data. The best choice depends on the data we research.

# # Summary
# 
# We compared **box plots**, **histograms**, and **kernel density estimators (KDEs)** as methods for visualizing data distributions, highlighting their strengths and limitations.
# 
# 1. **Box Plot**:
#    - **Strengths**: Summarizes key statistics (min, Q1, median, Q3, max), useful for spotting central tendency and outliers.
#    - **Limitations**: Loses details about the distribution shape (e.g., skewness, multimodality) and can be misleading with small datasets.
# 
# 2. **Histogram**:
#    - **Strengths**: Displays frequency distributions and spread, useful for seeing skewness, modality, and clusters in data.
#    - **Limitations**: Sensitive to bin size, lacks smoothness compared to KDEs, and can be cluttered when comparing multiple distributions.
# 
# 3. **KDE**:
#    - **Strengths**: Smoothly shows the underlying trend of the data distribution, avoiding the step-like appearance of histograms. Violin plots (KDE-based) combine features of both KDE and box plots.
#    - **Limitations**: Highly sensitive to bandwidth selection, does not show exact frequencies, and can be computationally intensive for large datasets.
# 
# Each method has its advantages and trade-offs, and the best choice depends on the specific analysis goal, whether it's summarizing data, visualizing spread, or understanding the overall trend.

# Chat:https://chatgpt.com/share/66f1d9ba-b0f8-800e-bca1-611f20c5f4e7
...
# Here are the sample means and sample variances for the datasets:
# 
# - **Data1**: Mean = 5.03, Variance = 8.71
# - **Data2**: Mean = 5.00, Variance = 2.25
# - **Data3**: Mean = 4.99, Variance = 9.19
# - **Data4**: Mean = 5.99, Variance = 0.27

# 1. A and C

# 2. B and A or B and C

# 3. I think there are not two data have similar variances but quite different means. If I had to choose the answer, I think B and D

# 4. C and D

# Chat:https://chatgpt.com/share/66f1f6c7-4218-800e-ae1c-be2846658c07
...
#      - Characterized by a long tail on the right (larger values).
#      - **Mean > Median**: Extreme high values pull the mean upward while the median remains closer to the center of the bulk of data.
#    - **Left-Skewed (Negatively Skewed)**:
#      - Characterized by a long tail on the left (smaller values).
#      - **Mean < Median**: Extreme low values pull the mean downward while the median stays closer to the center of the data.
# 
# ### Code Explanation:
# - The provided code uses the Gamma distribution to generate two samples:
#   - **Sample1**: Right-skewed data from a Gamma distribution.
#   - **Sample2**: Left-skewed data created by negating values from the Gamma distribution.
# - The code calculates the mean and median for both samples, demonstrating how they differ due to skewness.
# - Histograms are plotted to visually show the skewness of the data.
# 
# ### Conclusion:
# - The mean and median provide different insights into the center of a dataset, especially in the presence of skewness. The mean is affected by extreme values, while the median provides a more stable representation of central tendency in skewed distributions.
# 
# This discussion helps highlight the importance of understanding data distribution when analyzing datasets, as it influences interpretations of central tendency.

# Chat:https://chatgpt.com/share/66f1f91d-a3b0-800e-aa81-f238e282d67e
...
# 1.use pandas to make data table.
...
# 2.There is no missing data so I continue to the next step.

# 3.Use ChatGpt to visualize the data. Chat gave me this:
...
grouped_df = df.groupby('Year')[['Flowers', 'EveningOut', 'Clothing', 'GiftCards']].sum().reset_index()

# Plotting the data
plt.figure(figsize=(12, 6))
plt.plot(grouped_df['Year'], grouped_df['Flowers'], label='Flowers', marker='o')
plt.plot(grouped_df['Year'], grouped_df['EveningOut'], label='Evening Out', marker='o')
plt.plot(grouped_df['Year'], grouped_df['Clothing'], label='Clothing', marker='o')
plt.plot(grouped_df['Year'], grouped_df['GiftCards'], label='Gift Cards', marker='o')

# Adding titles and labels
plt.title('Spending Trends Over the Years')
plt.xlabel('Year')
plt.ylabel('Total Spending')
plt.xticks(grouped_df['Year'], rotation=45)
plt.legend()
plt.grid()
plt.tight_layout()
...
# Create a histogram for each spending category
plt.subplot(2, 2, 1)
plt.hist(df['Flowers'], bins=15, color='skyblue', edgecolor='black')
plt.title('Distribution of Spending on Flowers')
plt.xlabel('Spending Amount')
plt.ylabel('Frequency')

plt.subplot(2, 2, 2)
plt.hist(df['EveningOut'], bins=15, color='lightgreen', edgecolor='black')
plt.title('Distribution of Spending on Evening Out')
plt.xlabel('Spending Amount')
plt.ylabel('Frequency')

plt.subplot(2, 2, 3)
plt.hist(df['Clothing'], bins=15, color='salmon', edgecolor='black')
plt.title('Distribution of Spending on Clothing')
plt.xlabel('Spending Amount')
plt.ylabel('Frequency')

plt.subplot(2, 2, 4)
plt.hist(df['GiftCards'], bins=15, color='gold', edgecolor='black')
plt.title('Distribution of Spending on Gift Cards')
plt.xlabel('Spending Amount')
plt.ylabel('Frequency')
...
# 4.Conclude all the data and grapha.

# # Summary
# 1. **Dataset Exploration**: You are working with a dataset containing historical spending data, specifically focusing on the columns `Flowers`, `EveningOut`, `Clothing`, and `GiftCards`, along with the `Year` column.
# 
# 2. **Data Analysis**:
#    - You wanted to analyze spending trends over the years, for which I provided Python code to group the data by `Year` and visualize it using a line plot.
#    - Subsequently, you requested to visualize the same spending data using histograms, and I provided code to create histograms for each spending category, showing their distributions.
...
bn['percent change'] = bn['percent'].diff()
new_name = [True] + list(bn.name[:-1].values != bn.name[1:].values)
bn.loc[new_name, 'percent change'] = bn.loc[new_name, 'percent']  # Corrected column name
...
# Show the figure
fig.show(renderer="png")  # Use renderer="png" for GitHub and MarkUs submissions
